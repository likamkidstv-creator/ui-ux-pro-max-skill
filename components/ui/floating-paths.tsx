...
function FloatingPaths({ position }: { position: number }) {
  const paths = useMemo(() => {
    return Array.from({ length: 24 }, (_, i) => {
      ...
      const randomDashLength = 60 + seededRandom(seed) * 80
      const randomGap = 150 + seededRandom(seed + 1) * 100
      const randomDuration = 8 + seededRandom(seed + 2) * 12
      const randomDelay = seededRandom(seed + 3) * 10
